...
Object.assign(ICONS,{
 'arrow-top':'<path d="M4 3h16M7 12l5-5 5 5M12 7v14"/>',
 pin:'<path d="m16 3 5 5-4 2-3 6-3-3-6 6-1-1 6-6-3-3 6-3Z"/>',
 logout:'<path d="M9 4H4v16h5M14 8l4 4-4 4M8 12h10"/>',
 key:'<circle cx="8" cy="8" r="5"/><path d="m12 12 9 9m-5-5 2-2m-5-1 2-2"/>',
...
function canInviteScopedMembers(){return !!(window.__worktree_is_cloud_workspace&&currentAccount()?.role==='manager');}
function canOpenPeopleAccess(){return isAdmin()||canInviteScopedMembers();}
function inManagerInviteScope(nodeId,a=currentAccount()){
 if(!a||a.role!=='manager'||!Array.isArray(a.scopes)||!a.scopes.length)return false;
...
function roleDescription(r){
 const descriptions = {
  ...
  manager: 'Xem, tạo, giao, chỉnh sửa công việc và mời member/viewer trong nhánh được cấp.',
...
function matrixHTML(){
 const rows=[['Xem công việc','Toàn bộ','Trong nhánh','Việc được giao','Trong nhánh'],['Tạo, giao, sửa kế hoạch','yes','yes','no','no'],['Cập nhật, checklist, ghi giờ','yes','yes','Việc được giao','no'],['Xóa công việc','yes','Trong nhánh*','no','no'],['Mời tài khoản mới','yes','member/viewer trong nhánh','no','no'],['Sửa cây, quản trị tài khoản','yes','no','no','no'],['Nhập / sao lưu JSON','yes','no','no','no'],['Xuất CSV theo bộ lọc','yes','yes','no','no'],[T.pins+' · '+T.personal,'yes','yes','yes','yes']];
...
async function renderCloudEmployeeDirectory(forceReload = false) {
 ...
 const filtered = cloudEmployeesWithStatus.filter(emp => {
  if(canInviteScopedMembers()&&!inManagerInviteScope(emp.home_node_id))return false;
 ...
 const scopedManager = canInviteScopedMembers();
 ...
 container.innerHTML = filtered.map(emp => {
  ...
  if(emp.accountStatus === 'linked'){
   ...
   opsHTML = scopedManager
    ? `<button class="btn btn-sm secondary" data-v8="employee-invite" data-employee-id="${emp.id}">${icon('mail')} Mời vào nhánh</button>`
    : `
       <button class="btn btn-sm" data-v8="employee-detail" data-employee-id="${emp.id}" title="Xem chi tiết">${icon('eye')} Chi tiết</button>
       <button class="btn btn-sm" data-v8="employee-edit" data-employee-id="${emp.id}" title="Chỉnh sửa hồ sơ nhân sự">${icon('edit')} Sửa</button>
  ...
  return `<article class="account-row ${emp.accountStatus === 'suspended' ? 'is-suspended' : ''}">
   ...
   <div class="account-main" style="cursor:pointer;" data-v8="employee-detail" data-employee-id="${emp.id}" title="Xem thông tin chi tiết">
    ...
    <div style="margin-top:4px;display:flex;flex-wrap:wrap;align-items:center;gap:6px">
     ${statusBadge}
     ${emp.zalo_chat_id ? `<span class="badge" style="background:var(--green-soft);color:var(--green);border:1px solid rgba(16,115,83,0.2);font-size:11px;font-weight:600;padding:2px 7px;border-radius:4px;display:inline-flex;align-items:center;gap:4px;" title="Đã liên kết Zalo Bot (Chat ID: ${esc(emp.zalo_chat_id)})">${icon('bot')} Zalo đã kết nối</span>` : (emp.phone ? `<span class="badge" style="background:var(--surface-3);color:var(--muted);font-size:11px;font-weight:500;padding:2px 7px;border-radius:4px;display:inline-flex;align-items:center;gap:4px;" title="Số điện thoại: ${esc(emp.phone)} - Chưa kích hoạt Zalo">${icon('phone')} ${esc(emp.phone)}</span>` : '')}
    </div>
   </div>
...
function buildScopeTreeOptions(selectedIds = [], inputName = 'empScope') {
 ...
 const availableNodes = (typeof data !== 'undefined' && Array.isArray(data?.nodes)) ? data.nodes : (window.appState?.nodes || []);
 const rawNodes = canInviteScopedMembers() ? availableNodes.filter(n=>inManagerInviteScope(n.id)) : availableNodes;
...
function openScopedInviteDialog(){
 if(!requireLogin()||!canInviteScopedMembers())return deny();
 const scopedNodes=(data?.nodes||[]).filter(n=>inManagerInviteScope(n.id));
 if(!scopedNodes.length)return toast('Bạn chưa được cấp phạm vi để mời thành viên.','error');
 const preferred=scopedNodes.find(n=>String(n.id)===String(state.selected))||scopedNodes[0];
 const homeOptions=scopedNodes.map(n=>`<option value="${n.id}" ${n.id===preferred.id?'selected':''}>${esc(pathName(n.id))}</option>`).join('');
 const scopePickerHTML=buildScopeTreeOptions([preferred.id],'scopedInviteScope');

 $('accountContent').innerHTML=`
  <form id="scopedInviteForm">
   ${dialogHead('Mời thành viên vào phạm vi','accountTitle','accountDialog')}
   <div class="dialog-scroll">
    <div class="form-error" id="scopedInviteError" role="alert" tabindex="-1" hidden></div>
    <p class="view-note">Bạn chỉ có thể mời Nhân viên hoặc Chỉ xem vào các nhánh đang quản lý.</p>
    <div class="form-grid">
     ${field('Họ và tên *','scopedInviteName','text','','required maxlength="180" autocomplete="name"')}
     ${field('Email đăng nhập *','scopedInviteEmail','email','','required maxlength="180" placeholder="email@congty.com" autocomplete="email"')}
     <label class="field">Vai trò *<select id="scopedInviteRole"><option value="member">Nhân viên</option><option value="viewer">Chỉ xem</option></select></label>
     <label class="field">Phòng ban / dự án trực thuộc *<select id="scopedInviteHome">${homeOptions}</select></label>
     <div class="field full">
      <div class="scope-label"><strong>Phạm vi được cấp</strong><small id="scopedInviteScopeCount"></small></div>
      <p class="field-hint">Mọi nhánh được chọn phải nằm trong phạm vi bạn đang quản lý.</p>
      <input type="search" id="scopedInviteScopeSearch" placeholder="Tìm nhánh..." aria-label="Tìm nhánh phân quyền">
      <div class="scope-picker">${scopePickerHTML}</div>
     </div>
     <div class="role-explanation full" id="scopedInviteRoleExplanation"></div>
    </div>
   </div>
   <div class="dialog-foot"><button type="button" class="btn" data-action="close" data-dialog="accountDialog">Hủy</button><button type="submit" class="btn primary" id="scopedInviteSubmit">Tạo lời mời</button></div>
  </form>`;

 const refreshHint=()=>{
  const role=$('scopedInviteRole').value;
  $('scopedInviteRoleExplanation').innerHTML=`<span class="role-badge role-${role}">${T[role]}</span><p>${roleDescription(role)}</p>`;
  $('scopedInviteScopeCount').textContent=$$('#scopedInviteForm [name="scopedInviteScope"]:checked').length+' nhánh';
 };
 $('scopedInviteRole').addEventListener('change',refreshHint);
 $('scopedInviteScopeSearch').addEventListener('input',e=>{const q=fold(e.target.value);$$('#scopedInviteForm .scope-option').forEach(el=>el.hidden=!el.dataset.scopeName.includes(q));});
 $$('#scopedInviteForm [name="scopedInviteScope"]').forEach(el=>el.addEventListener('change',refreshHint));
 $('scopedInviteHome').addEventListener('change',e=>{
  const checked=$$('#scopedInviteForm [name="scopedInviteScope"]:checked');
  if(!checked.length){const cb=$(`#scopedInviteForm [name="scopedInviteScope"][value="${e.target.value}"]`);if(cb)cb.checked=true;refreshHint();}
 });
 refreshHint();

 $('scopedInviteForm').addEventListener('submit',async e=>{
  e.preventDefault();
  const btn=$('scopedInviteSubmit'),err=$('scopedInviteError');
  if(btn.disabled)return;btn.disabled=true;err.hidden=true;
  try{
   const organizationId=window.appState?.activeOrganizationId;
   const fullName=$('scopedInviteName').value.trim();
   const email=$('scopedInviteEmail').value.trim().toLowerCase();
   const homeNodeId=$('scopedInviteHome').value;
   const role=$('scopedInviteRole').value;
   const scopeNodeIds=$$('#scopedInviteForm [name="scopedInviteScope"]:checked').map(el=>el.value);
   const token=await window.EmployeeService.inviteScopedMember({organizationId,email,fullName,homeNodeId,role,scopeNodeIds:scopeNodeIds.length?scopeNodeIds:[homeNodeId]});
   closeDialog('accountDialog',true);$('accountContent').innerHTML='';await renderCloudEmployeeDirectory(true);
   openInviteLinkDialog({token,email,fullName,role,homeNodeName:nodeName(homeNodeId)});
  }catch(error){err.textContent=error.message||'Không thể tạo lời mời.';err.hidden=false;err.focus({preventScroll:true});}
  finally{btn.disabled=false;}
 });
 showDialog('accountDialog','#scopedInviteName');
}

function openInviteExistingEmployeeDialog(employeeId){
 if(!requireLogin()||!canOpenPeopleAccess()) return deny();
 const scopedManager=canInviteScopedMembers();
 const emp = cloudEmployeesWithStatus.find(e => e.id === employeeId);
 if(!emp) return toast('Không tìm thấy nhân sự.', 'error');
 if(scopedManager&&!inManagerInviteScope(emp.home_node_id))return deny();
 if(emp.accountStatus === 'linked'){
    ...
    <div class="form-grid">
     ...
     <label class="field">
      ...
      <select id="inviteExistRole">
       <option value="member" selected>Nhân viên</option>
       <option value="viewer">Chỉ xem</option>
       ${scopedManager?'':'<option value="manager">Quản lý</option><option value="admin">Quản trị viên</option>'}
      </select>
     </label>
 ...
 $('inviteExistingForm').addEventListener('submit', async e => {
  ...
  try{
   ...
   if(scopedManager&&!['member','viewer'].includes(r)) throw new Error('Quản lý chỉ có thể mời Nhân viên hoặc Chỉ xem.');

   const scopes = r === 'admin' ? [] : $$('#inviteExistingForm [name="inviteExistScope"]:checked').map(el => el.value);

   const inviteMethod=scopedManager?'inviteScopedMember':'inviteExistingEmployee';
   const token = await window.EmployeeService[inviteMethod]({
    organizationId: orgId,
    employeeId: emp.id,
    email,
  ...
  <div class="dialog-scroll">
   ...
   <div style="margin:16px 0 10px;font-weight:700;color:var(--text);font-size:13px;border-bottom:1px solid var(--line);padding-bottom:6px;display:flex;align-items:center;justify-content:space-between;">
    <div style="display:flex;align-items:center;gap:6px;">
     ${icon('bot')} <span>Thông báo qua Zalo Bot</span>
    </div>
    ${emp.zalo_chat_id ? `
   ...
   <div style="background:var(--surface);padding:14px;border-radius:8px;border:1px solid var(--line);font-size:13px;margin-bottom:16px;">
    ...
    ` : `
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
       ...
       <button type="button" class="btn btn-sm primary" data-v8="employee-guide-zalo" data-employee-id="${emp.id}" data-employee-phone="${esc(emp.phone||'')}" data-employee-name="${esc(emp.full_name||'')}">${icon('bot')} Hướng dẫn liên kết Zalo</button>
      </div>
    `}
   </div>
...
function openZaloGuideDialog(phone, name) {
 ...
 $('infoContent').innerHTML = `
  <div style="padding:16px 20px;">
   <div style="background:var(--surface-2);border:1px solid var(--line);border-radius:12px;padding:16px;margin-bottom:16px;">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
     <div style="width:46px;height:46px;border-radius:12px;background:var(--primary-soft);color:var(--primary);display:flex;align-items:center;justify-content:center;border:1px solid rgba(105,98,219,0.22);box-shadow:0 2px 8px rgba(105,98,219,0.08);flex-shrink:0;">
      <svg class="icon" style="width:24px;height:24px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
       <path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/>
      </svg>
     </div>
     <div style="flex:1;min-width:0;">
      <div style="display:flex;align-items:center;gap:8px;">
       <strong style="font-size:15px;color:var(--text);font-weight:700;">${esc(botName)}</strong>
       <span style="display:inline-flex;align-items:center;padding:1px 7px;border-radius:10px;font-size:11px;font-weight:600;background:var(--green-soft);color:var(--green);">Online</span>
      </div>
      <div class="muted" style="font-size:12px;margin-top:2px;">Tài khoản chính thức: <code>@${esc(botAccount)}</code></div>
     </div>
    </div>
   ...
   <div style="font-size:13px;color:var(--text);line-height:1.7;">
    <strong style="display:block;margin-bottom:8px;font-size:14px;color:var(--text);">Quy trình 3 bước cực kỳ đơn giản:</strong>
   ...
   <div style="display:flex;gap:10px;margin-top:20px;justify-content:flex-end;">
    ${phone ? `<button type="button" class="btn" id="btnCopyEmpPhone">${icon('copy')} Sao chép SĐT</button>` : ''}
    <button type="button" class="btn primary" id="btnOpenZaloBotDirect">${icon('arrow-up-right')} Mở Zalo Bot ngay</button>
   </div>
  </div>
 `;
...
function openAccess(tab=accessTab){
 if(!requireLogin()||!canOpenPeopleAccess())return deny();
 const scopedManager=canInviteScopedMembers();
 if(scopedManager&&tab==='audit')tab='accounts';
 accessTab=tab;$('accessTitle').textContent=scopedManager?'Thành viên trong phạm vi':(window.__worktree_is_cloud_workspace ? 'Nhân sự & tài khoản' : T.accounts);
 const isCloud = !!window.__worktree_is_cloud_workspace;
 ...
 const tabs=scopedManager?[['accounts','Thành viên'],['matrix',T.matrix]]:[['accounts',isCloud ? 'Nhân sự' : T.accounts],['matrix',T.matrix],['audit',T.audit]];
 const primaryAction=scopedManager
  ? `<button class="btn primary" data-v8="employee-invite-new">${icon('mail')}Mời thành viên</button>`
  : `<button class="btn primary" data-v8="${isCloud ? 'employee-new' : 'account-new'}">${icon('plus')}${isCloud ? 'Thêm nhân viên' : T.create}</button>`;
 
 $('accessContent').innerHTML=`<div class="access-banner"><div><span class="access-banner-icon">${icon('shield')}</span><h3>Rõ vai trò. Đúng phạm vi.</h3><p>${scopedManager?'Mời Nhân viên hoặc Chỉ xem vào đúng nhánh bạn đang quản lý.':isCloud ? 'Quản lý nhân sự và phân quyền truy cập không gian làm việc.' : 'Cấp quyền theo công việc thực tế, không chia sẻ một tài khoản chung.'}</p></div><div class="access-stat"><strong>${count}</strong><span>${scopedManager?'Trong phạm vi':isCloud ? 'Nhân sự' : T.active}</span></div></div><div class="access-tabs" role="tablist">${tabs.map(([key,label])=>`<button role="tab" aria-selected="${key===tab}" data-v8="access-tab" data-tab="${key}">${label}</button>`).join('')}</div><div id="accessPanel" role="tabpanel">${tab==='matrix'?matrixHTML():tab==='audit'?accessAuditHTML():`<div class="access-toolbar"><label class="access-search">${icon('search')}<input id="accountSearch" type="search" value="${esc(accountQuery)}" placeholder="${isCloud ? 'Tìm nhân sự, chức danh...' : T.searchAccounts}" aria-label="${isCloud ? 'Tìm nhân sự' : T.searchAccounts}"></label>${primaryAction}</div><div id="accountList"></div><p class="view-note">${scopedManager?'Lời mời và phạm vi được kiểm tra lại tại cơ sở dữ liệu. Bạn không thể cấp quyền cao hơn hoặc vượt khỏi nhánh quản lý.':isCloud ? 'Tài khoản được bảo vệ bởi Supabase Auth & Row-Level Security đa tổ chức.' : T.credentialsHint}</p>`}</div>${isCloud ? '' : `<details class="access-limits"><summary>${icon('info')}${T.local}</summary><p>${T.localWarning}</p></details>`}`;
...
function permissionForAction(action,id){
 ...
 if(action==='access')return canOpenPeopleAccess();
...
function applyPermissionUI(){
 ...
 $('accessNav').hidden=!canOpenPeopleAccess();
 ...
 if(v){
  ...
  Promise.resolve().then(async()=>{
   switch(action){
    ...
    case 'employee-invite-new':openScopedInviteDialog();break;
