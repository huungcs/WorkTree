...
async function saveAccount(e){
 ...
 try{
  ...
  const original=identity.accounts.find(a=>a.id===accountEditing);if(original?.id===session.id&&role!=='admin')throw Error(T.selfGuard);
  ...
  if(a.id===session.id){session.version=a.version;saveSession();state.currentUser=a.personId;}
...
async function toggleAccount(id){
 if(!requireAdmin())return;const a=identity.accounts.find(a=>a.id===id);if(!a)return;if(a.id===session.id)return toast(T.selfGuard,'error');
...
async function savePassword(e,forced,reset){
 ...
 try{
  ...
  const id=forced?pendingUser.id:passwordTarget||session.id,a=identity.accounts.find(a=>a.id===id);
  ...
  if(forced)await enterWorkspace(u);else if(id===session.id){session.version=u.version;saveSession();}
...
function savePins(list,remember=true){
 ...
 try{
  ...
  const before=ownPins(),next=clone(pinDB);next.users[session.id]=normalPins(list);const packed=JSON.stringify(next);localStorage.setItem(PIN_KEY,packed);pinDB=next;pinRaw=packed;if(remember)pinUndo={user:session.id,list:before};renderPins();decoratePinButtons();return true;
 ...
 if(v){
  ...
  Promise.resolve().then(async()=>{
   switch(action){
    ...
    case 'pin-undo':if(pinUndo?.user===session.id){const list=pinUndo.list;if(savePins(list,false)){pinUndo=null;openPins();renderTree();}}break;
   }
