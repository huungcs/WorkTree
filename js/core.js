...
async function openNotificationSettings() {
 ...
 try {
  ...
  if (reminders.length === 0) {
   reminderListHtml = `
    <div style="text-align:center;padding:22px 14px;border-radius:10px;border:1px dashed var(--line);background:var(--surface-2);color:var(--muted);font-size:11px;line-height:1.6">
     <div style="font-size:20px;margin-bottom:4px">📅</div>
     <strong>Chưa có lịch nhắc việc nào được tạo</strong>
     <p style="margin:2px 0 0">Hãy điền nội dung và thời gian ở trên để hệ thống tự động thông báo và đổ chuông.</p>
    </div>
   `;
  } else {
   reminderListHtml = `<div style="display:grid;gap:8px;max-height:280px;overflow:auto;padding-right:2px">`;
   reminders.forEach(r => {
    ...
    const statusBadge = isPending
     ? `<span class="tag" style="background:var(--amber-soft);color:var(--amber);font-size:9.5px;font-weight:600">Đang chờ</span>`
     : isCompleted
     ? `<span class="tag" style="background:var(--green-soft);color:var(--green);font-size:9.5px;font-weight:600">Đã gửi</span>`
     : `<span class="tag" style="background:var(--surface-3);color:var(--muted);font-size:9.5px">Đã hủy</span>`;

    reminderListHtml += `
     <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-radius:8px;border:1px solid ${isPending ? 'var(--line-strong)' : 'var(--line)'};background:${isPending ? 'var(--surface-2)' : 'var(--surface)'};gap:10px">
      <div style="min-width:0;flex:1">
       <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;flex-wrap:wrap">
        <strong style="font-size:12px;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(r.title)}</strong>
        ${statusBadge}
       </div>
       <div style="font-size:10.5px;color:var(--muted);display:flex;align-items:center;gap:5px;flex-wrap:wrap">
        <span style="display:inline-flex;align-items:center">${icon('clock')}</span>
        <span>${dateStr}</span>
        ${timeRel ? `<span style="color:${isPending ? 'var(--amber)' : 'var(--muted)'};font-weight:${isPending ? '600' : '400'}">${timeRel}</span>` : ''}
       </div>
      </div>
      <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
       ${isPending ? `
        <button class="btn small" data-action="cancel-manual-reminder" data-id="${r.id}" title="Hủy lịch nhắc này">
         ${icon('x')}Hủy
        </button>
       ` : ''}
       <button class="icon-btn small" data-action="delete-manual-reminder" data-id="${r.id}" title="Xóa lịch nhắc" style="color:var(--muted)">
        ${icon('trash')}
       </button>
      </div>
     </div>
  ...
  $('infoContent').innerHTML = `
   ...
   <section class="settings-section" style="margin-bottom:16px">
    ...
    <p style="font-size:11px;color:var(--muted);margin-bottom:10px">Hệ thống sẽ gửi thông báo và đổ chuông theo đúng thời gian bạn đã chọn.</p>
    <div style="display:grid;gap:8px">
     <input id="manualReminderTitle" class="field" placeholder="Nội dung cần nhắc (ví dụ: Họp giao ban, nộp báo cáo...)" maxlength="240" style="width:100%;box-sizing:border-box;padding:8px 10px;border-radius:6px;border:1px solid var(--line-strong);font-size:11px;background:var(--surface);color:var(--text)">
     <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
      <input id="manualReminderTime" type="datetime-local" class="field" style="flex:1;min-width:180px;box-sizing:border-box;padding:8px 10px;border-radius:6px;border:1px solid var(--line-strong);font-size:11px;background:var(--surface);color:var(--text)">
      <div style="display:flex;gap:4px;flex-wrap:wrap">
       <button class="btn small" data-action="quick-reminder-time" data-minutes="15" title="Hẹn sau 15 phút">+15p</button>
       <button class="btn small" data-action="quick-reminder-time" data-minutes="30" title="Hẹn sau 30 phút">+30p</button>
       <button class="btn small" data-action="quick-reminder-time" data-minutes="60" title="Hẹn sau 1 giờ">+1h</button>
       <button class="btn small" data-action="quick-reminder-time" data-preset="tomorrow_morning" title="Hẹn 08:30 sáng mai">Sáng mai</button>
      </div>
     </div>
   ...
   <section class="settings-section" style="margin-bottom:16px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:8px">
     <h3 style="margin:0">${icon('calendar')}Lịch nhắc hẹn cá nhân (${reminders.length})</h3>
     <div style="display:flex;gap:6px">
      <span class="tag" style="background:var(--amber-soft);color:var(--amber);font-size:10px">${pendingReminders.length} đang chờ</span>
      <span class="tag" style="background:var(--green-soft);color:var(--green);font-size:10px">${completedReminders.length} đã nhắc</span>
     </div>
    </div>
    ${reminderListHtml}
   </section>
