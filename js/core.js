...
function legacyRenderTree(){
 ...
 $('orgTree').innerHTML=row(rootNode(),0)||(q?'<div class="tree-empty">Không tìm thấy đơn vị phù hợp.</div>':'');
...
function renderShell(arr){
 const node=byNode.get(state.selected),root=rootNode(),user=person();
...
function openNodeForm(id=null,type=null){
 ...
 $('nType').innerHTML=Object.entries(TYPES).filter(([key])=>isRoot||key!=='company').map(([key,label])=>`<option value="${key}" ${key===(n?.type||type||(state.view==='workload'?'person':'department'))?'selected':''}>${esc(label)}</option>`).join('');
