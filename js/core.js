 ...
 if(window.__worktree_is_cloud_workspace && window.NotificationService){
  ...
  try {
   ...
   if(notifications.length === 0){
    if(pendingReminders.length > 0){
     html += emptyState('Chưa có thông báo mới', 'Bạn có ' + pendingReminders.length + ' lịch nhắc việc đang đếm ngược ở trên. Khi đến giờ hệ thống sẽ đổ chuông và gửi thông báo.', null, '', false, 'bell');
    } else {
